...
set.seed(2)
voterMeckCo <- voterMeckCo %>% sample_n(1000)
...
#voterMeck$E1_20191008 <- as.factor(voterMeck$E1_20191008)


voterMeck <- mutate(voterMeck, E2_20190910 = 0)
voterMeck <- mutate(voterMeck, E3_20190514 = 0)
voterMeck <- mutate(voterMeck, E4_20181106 = 0)
voterMeck <- mutate(voterMeck, E5_20180508 = 0)
voterMeck <- mutate(voterMeck, E6_20171107 = 0)
voterMeck <- mutate(voterMeck, E7_20170912 = 0)
voterMeck <- mutate(voterMeck, E8_20161108 = 0)
voterMeck <- mutate(voterMeck, E9_20160315 = 0)
voterMeck <- mutate(voterMeck, E10_20151103 = 0)
voterMeck <- mutate(voterMeck, E11_20151006 = 0)
voterMeck <- mutate(voterMeck, E12_20150915 = 0)
voterMeck <- mutate(voterMeck, E13_20141104 = 0)
voterMeck <- mutate(voterMeck, E14_20140506 = 0)
voterMeck <- mutate(voterMeck, E15_2013110 = 0)
voterMeck <- mutate(voterMeck, E16_20131008 = 0)
voterMeck <- mutate(voterMeck, E17_20130910 = 0)
voterMeck <- mutate(voterMeck, E18_20121106 = 0)
voterMeck <- mutate(voterMeck, E19_20120717 = 0)
voterMeck <- mutate(voterMeck, E20_20120508 = 0)
...
filterTest <- voterMeckCo1 %>% filter(!is.na(E1_date))
print(filterTest)
filterTest2 <- voterMeck %>% filter(E1_date == "10/08/2019")


#voterMeck <- mutate(voterMeck, e1_20191008 =  NULL)
voterMeck <- mutate(voterMeck, E1_20191008 = ifelse(E1_date == "10/08/2019", "1",
                                               ifelse(registr_dt > as.Date('2019-09-13'), "NA", "0")))



print(levels(as.factor(voterMeck$E1_20191008)))
  
testa <- filter(voterMeck, E1_date == as.Date("10/08/2019"))
testb <- filter(voterMeck, voterMeck$registr_dt <= as.Date("2019-09-13"))
testc <- filter(voterMeck, voterMeck$registr_dt > as.Date("2019-09-13"))
testd <- filter(voterMeck, voterMeck$registr_dt <= as.Date(as.character(eDateMin25[[1]])))
as.Date(as.character
1964-05-01

test0 <- filter(voterMeck, E1_20191008 == "0")
test1 <- filter(voterMeck, E1_20191008 == "1")
test2 <- filter(voterMeck, is.na(E1_20191008))

test0a <- filter(voterMeck, voterMeck$registr_dt <= eDateMin25[[1]])
& E1_date != "10/08/2019")

eDateMin25[[1]]


test2a  <- voterMeck %>% filter(registr_dt > eDateMin25[[1]]) 
test2ab <- voterMeck %>% filter(E1_date != "10/08/2019") 

test2d <- filter(voterMeck, is.na(E1_20191008))
...
>>>>>>> 9a7841be186e373c567295379826678756f8cbd0
